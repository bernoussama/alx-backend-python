...
# again in the *__enter__* method of the BaseFileLock, but not released
# again automatically. issue #37 (memory leak)
...
class FileLockContext:
    ...
    # When the lock is acquired is increased and the lock is only
    # released, when this value is 0
    lock_counter: int = 0
...
class BaseFileLock(ABC, contextlib.ContextDecorator):
    ...
    def acquire(
        ...
        *,
        ...
        # Increment the number right at the beginning. We can still undo
        ...
        try:
            while True:
                ...
                _LOGGER.debug(
                    msg, lock_id, lock_filename, poll_interval)
                time.sleep(poll_interval)
        except BaseException:  # Something did go wrong, so decrement the counter.
            self._context.lock_counter = max(
                0, self._context.lock_counter - 1)
            raise
