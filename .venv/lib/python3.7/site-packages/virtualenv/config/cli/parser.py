...
class VirtualEnvOptions(Namespace):
    ...
    def __repr__(self) -> str:
        return f"{type(self).__name__}({', '.join(f'{k}={v}' for k, v in vars(
...
class VirtualEnvConfigParser(ArgumentParser):
    ...
    def _fix_default(self, action):
        if hasattr(
                ...
                "default") and hasattr(
                action,
                "dest") and action.default != SUPPRESS:
            as_type = get_type(action)
            names = OrderedDict((i.lstrip("-").replace("-", "_"), None)
    ...
    def _get_help_string(self, action):
        ...
        if hasattr(action, "default_source"):
            ...
            if text.endswith(default):
                text = f"{text[: -
                               len(default)]} (default: %(default)s -> from %(default_source)s)"
        return text
