...
class Creator(ABC):
    ...
    def validate_dest(cls, raw_value):  # noqa: C901
        ...
        def non_write_able(dest, value):
            common = Path(
                *os.path.commonprefix([value.parts, dest.parts]))
            msg = f"the destination {
                dest.relative_to(common)} is not write-able at {common}"
            raise ArgumentTypeError(msg)

        # the file system must be able to encode
        # note in newer CPython this is always utf-8
        ...
        if refused:
            ...
            msg = f"the file system codec ({encoding}) cannot handle characters {
                bad!r} within {
                raw_value!r}"
            raise ArgumentTypeError(msg)
        if os.pathsep in raw_value:
            msg = (
                f"destination {
                    raw_value!r} must not contain the path separator ({
                    os.pathsep})" f" as this would break the activation scripts")
            raise ArgumentTypeError(msg)
    ...
    def setup_ignore_vcs(self):
        ...
        # mark this folder to be ignored by VCS, handle
        # https://www.python.org/dev/peps/pep-0610/#registered-vcs
        ...
        # Subversion - does not support ignore files, requires direct
        # manipulation with the svn tool
