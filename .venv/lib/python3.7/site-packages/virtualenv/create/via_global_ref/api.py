...
class ViaGlobalRefApi(Creator, ABC):
    ...
    def _should_symlink(options):
        ...
        copies, symlinks = getattr(
            options, "copies", False), getattr(
            options, "symlinks", False)
        copy_src, sym_src = options.get_source(
            "copies"), options.get_source("symlinks")
    ...
    def env_patch_text(self):
        ...
        with self.app_data.ensure_extracted(Path(__file__).parent / "_virtualenv.py") as resolved_path:
            ...
            return text.replace('"__SCRIPT_DIR__"', repr(
                os.path.relpath(str(self.script_dir), str(self.purelib))))

    def _args(self):
        return [
            *super()._args(),
            ("global",
             self.enable_system_site_package)]
