...
class PyPy(ViaGlobalRefVirtualenvBuiltin, abc.ABC):
    ...
    def _executables(cls, interpreter):
        ...
        targets = sorted(
            f"{name}{
                PyPy.suffix}" for name in cls.exe_names(interpreter))
    ...
    def exe_names(cls, interpreter):
        return {
            cls.exe_stem(), "python", f"python{
                interpreter.version_info.major}", f"python{
    ...
    def _add_shared_libs(cls, interpreter):
        ...
        python_dir = Path(
            interpreter.system_executable).resolve().parent
