...
class CPythonWindows(CPython, WindowsSupports, ABC):
    ...
    def _executables(cls, interpreter):
        ...
        # for more info on pythonw.exe see
...
def is_mac_os_framework(interpreter):
    if interpreter.platform == "darwin":
        return interpreter.sysconfig_vars.get(
            "PYTHONFRAMEWORK") == "Python3"
    return False


def is_macos_brew(interpreter):
    return interpreter.platform == "darwin" and _BREW.fullmatch(
        interpreter.system_prefix) is not None
