...
class CPython3Windows(CPythonWindows, CPython3):
    ...
    def setup_meta(cls, interpreter):
        if is_store_python(
                interpreter):  # store python is not supported here
            return None
    ...
    def shim(cls, interpreter):
        shim = Path(interpreter.system_stdlib) / "venv" / \
            "scripts" / "nt" / "python.exe"
        if shim.exists():
            return shim
        return None
    ...
    def host_python(cls, interpreter):
        if cls.has_shim(interpreter):
            ...
            # it also means the wrapper must be copied to avoid bugs
            # such as https://bugs.python.org/issue42013
