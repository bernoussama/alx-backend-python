...
class Builtin(Discover):
    ...
    def __repr__(self) -> str:
        spec = self.python_spec[0] if len(
            self.python_spec) == 1 else self.python_spec
        return f"{
            self.__class__.__name__} discover of python_spec={
            spec!r}"
...
def propose_interpreters(  # noqa: C901, PLR0912, PLR0915
    ...
    for py_exe in try_first_with:
        ...
        try:
            # Windows Store Python does not work with os.path.exists,
            # but does for os.lstat
            os.lstat(path)
        except OSError:
            pass
        else:
    ...
    if spec.path is not None:
        ...
        if IS_WIN:
            ...
            for interpreter in propose_interpreters(
                    spec, app_data, env):
    ...
    # finally just find on path, the path order matters (as the
    # candidates are less easy to control by end user)
...
class LazyPathDump:
    def __init__(self, pos: int, path: Path,
    ...
    def __repr__(self) -> str:
        ...
        if self.env.get(
                "_VIRTUALENV_DEBUG"):  # this is the over the board debug
            content += " with =>"
...
def path_exe_finder(
        spec: PythonSpec) -> Callable[[Path], Generator[tuple[Path, bool], None, None]]:
    ...
    def path_exes(
            path: Path) -> Generator[tuple[Path, bool], None, None]:
        # 4. then maybe it's something exact on PATH - if it was direct
        # lookup implementation no longer counts
        ...
        for exe in path.iterdir():
            ...
            if match:
                # the implementation must match when we find
                # “python[ver]”
