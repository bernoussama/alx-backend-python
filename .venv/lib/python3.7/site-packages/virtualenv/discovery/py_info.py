...
class PythonInfo:  # noqa: PLR0904
    ...
    def __init__(self) -> None:  # noqa: PLR0915
        ...
        # this is a tuple in earlier, struct later, unify to our own
        # named tuple
        ...
        self.version_nodot = sysconfig.get_config_var(
            "py_version_nodot")
        ...
        self.prefix = abs_path(
            getattr(
                sys,
                "prefix",
                None))  # prefix we think
        self.base_prefix = abs_path(
            getattr(sys, "base_prefix", None))  # venv
        ...
        if "venv" in scheme_names:
            ...
            self.sysconfig_paths = {
                i: sysconfig.get_path(
                    i,
                    expand=False,
                    scheme=self.sysconfig_scheme) for i in sysconfig.get_path_names()}
            # we cannot use distutils at all if "venv" exists, distutils
            # don't know it
        ...
        # intentionally select `posix_prefix` which is the unaltered
        # posix-like paths
            ...
            self.sysconfig_paths = {i: sysconfig.get_path(
                i, expand=False) for i in sysconfig.get_path_names()}
        ...
        self.system_stdlib_platform = self.sysconfig_path(
            "platstdlib", confs)
    ...
    def _fast_get_system_executable(self):
            ...
            if self.real_prefix is None:
                ...
                if base_executable is not None:  # noqa: SIM102 # use the saved system executable if present
                    if sys.executable != base_executable:  # we know we're in a virtual environment, cannot be us
                        ...
                        # the 'home' key from pyvenv.cfg which often
                        # points to the system install location.
                        major, minor = self.version_info.major, self.version_info.minor
                        if self.os == "posix" and (
                                major, minor) >= (3, 11):
                            # search relative to the directory of
        ...
        # note we must choose the original and not the pure executable
        # as shim scripts might throw us off
    ...
    def install_path(self, key):
        ...
        if result is None:  # use sysconfig if sysconfig_scheme is set or distutils is unavailable
            ...
            config_var = {
                k: "" if v in prefixes else v for k,
                v in self.sysconfig_vars.items()}
            result = self.sysconfig_path(
                key, config_var=config_var).lstrip(
                os.sep)
        return result
    ...
    def _distutils_install():
        ...
        # note here we don't import Distribution directly to allow
        # setuptools to patch it
        ...
        # paths generated are relative to prefix that contains the path
        # sep, this makes it relative
        i.prefix = os.sep
        i.finalize_options()
        return {
            key: (
    ...
    def system_include(self):
        ...
        if not os.path.exists(
                path):  # some broken packaging don't respect the sysconfig, fallback to distutils path
            # the pattern include the distribution name too at the end,
            # remove that via the parent call
            fallback = os.path.join(
                self.prefix, os.path.dirname(
                    self.install_path("headers")))
    ...
    @property
    def spec(self):
        return "{}{}-{}".format(self.implementation,
                                ".".join(str(i) for i in self.version_info),
                                self.architecture)

    @classmethod
    def clear_cache(cls, app_data):
        # this method is not used by itself, so here and called
        # functions can import stuff locally
    ...
    def satisfies(self, spec, impl_must_match):  # noqa: C901
        ...
        if spec.path:
            ...
            if not spec.is_abs:
                # if path set, and is not our original executable name,
                # this does not match
    ...
    def _to_dict(self):
        data = {
            var: (
                getattr(
                    self,
                    var) if var != "_creators" else None) for var in vars(self)}
    ...
    def _from_json(cls, payload):
        # the dictionary unroll here is to protect against pypy bug of
    ...
    def _resolve_to_system(cls, app_data, target):
        ...
        while target.system_executable is None:
            ...
            if prefix in prefixes:
                if len(prefixes) == 1:
                    # if we're linking back to ourselves accept
                    # ourselves with a WARNING
                    logging.info(
            ...
            target = target.discover_exe(
                app_data, prefix=prefix, exact=False)
    ...
    def discover_exe(self, app_data, prefix, exact=True, env=None):  # noqa: FBT002
        ...
        # we don't know explicitly here, do some guess work - our
        # executable name should tell
    ...
    def _select_most_likely(discovered, target):
        ...
        def sort_by(info):
            ...
            return sum((1 << pos if match else 0)
    ...
    def _possible_base(self):
        ...
        # python is always the final option as in practice is used by
        # multiple implementation as exe name
...
if __name__ == "__main__":
    ...
    sys.stdout.write(
        "".join((start_cookie[::-1], info, end_cookie[::-1])))
