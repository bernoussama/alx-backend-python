...
class PythonSpec:
                ...
                try:
                    ...
                    if version is not None:
                        ...
                        if len(versions) == 3:  # noqa: PLR2004
                            ...
                            major = int(
                                str(version_data)[0])  # first digit major
    ...
    def generate_re(self, *, windows: bool) -> re.Pattern:
        ...
        impl = "python" if self.implementation is None else f"python|{
            re.escape(
                self.implementation)}"
        ...
        # Try matching `direct` first, so the `direct` group is filled
        # when possible.
        return re.compile(
    ...
    def __repr__(self) -> str:
        ...
        return f"{name}({', '.join(f'{k}={getattr(self,
                                                  k)}' for k in params if getattr(self,
                                                                                  k) is not None)})"
