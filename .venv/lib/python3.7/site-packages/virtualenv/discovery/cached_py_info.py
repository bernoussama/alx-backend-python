...
def _get_from_cache(cls, app_data, exe, env, ignore_cache=True):  # noqa: FBT002
    ...
    # independent if it was from the file or in-memory cache fix the
    # original executable location
...
def _run_subprocess(cls, exe, app_data, env):
    py_info_script = Path(os.path.abspath(
        __file__)).parent / "py_info.py"
    ...
    with app_data.ensure_extracted(py_info_script) as py_info_script:
        ...
        # prevent sys.prefix from leaking into the child process - see
    ...
    if code == 0:
        ...
        # keep original executable as this may contain initialization
        # code
        result.executable = exe
    else:
        msg = f"{exe} with code {code}{
            f' out: {
                out!r}' if out else ''}{
            f' err: {
                err!r}' if err else ''}"
