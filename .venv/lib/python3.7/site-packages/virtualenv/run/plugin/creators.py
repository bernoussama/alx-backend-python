...
class CreatorSelector(ComponentBuilder):
    ...
    def for_interpreter(cls, interpreter):
        ...
        for key, creator_class in cls.options(
                "virtualenv.create").items():
            if key == "builtin":
            ...
            if describe is None and issubclass(
                    creator_class,
                    Describe) and creator_class.can_describe(interpreter):
    ...
    def add_selector_arg_parse(self, name, choices):
        # prefer the built-in venv if present, otherwise fallback to
        # first defined type
        choices = sorted(
            choices, key=lambda a: 0 if a == "builtin" else 1)
        ...
        self.parser.add_argument(
            ...
            required=False,
            help=f"create environment via{
                '' if self.builtin_key is None else f' (builtin = {
                    self.builtin_key})'}",
        )
