...
def session_via_cli(args, options=None, setup_logging=True, env=None):  # noqa: FBT002
    ...
    creator, seeder, activators = tuple(
        e.create(options) for e in elements)  # create types
    return Session(
        options.verbosity,
        options.app_data,
...
def load_app_data(args, parser, options):
    ...
    # here we need a write-able application data (e.g. the zipapp might
    # need this for discovery cache)
    parser.add_argument(
        "--app-data",
...
def add_version_flag(parser):
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {__version__} from {
            virtualenv.__file__}",
...
def _do_report_setup(parser, args, setup_logging):
    ...
    verbosity_group = parser.add_argument_group(
        title="verbosity", description=msg.format(
