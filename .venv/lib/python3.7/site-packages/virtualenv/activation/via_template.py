...
class ViaTemplateActivator(Activator, ABC):
    ...
    def _generate(self, replacements, templates, to_folder, creator):
        ...
        for template in templates:
            text = self.instantiate_template(
                replacements, template, creator)
            dest = to_folder / self.as_name(template)
            # remove the file if it already exists - this prevents permission
            # errors when the dest is not writable
            if dest.exists():
                dest.unlink()
            # Powershell assumes Windows 1252 encoding when reading
            # files without BOM
            encoding = "utf-8-sig" if str(
                template).endswith(".ps1") else "utf-8"
            # use write_bytes to avoid platform specific line
            # normalization (\n -> \r\n)
