...
def get_wheel(  # noqa: PLR0913
    ...
    # not all wheels are compatible with all python versions, so we need
    # to py version qualify it
    wheel = None
    ...
    for line in out.splitlines():
        ...
        for marker in ("Saved ", "File was already downloaded "):
            if stripped_line.startswith(marker):
                return Wheel(
                    Path(stripped_line[len(marker):]).absolute())
    # if for some reason the output does not match fallback to the
    # latest version with that spec
...
def pip_wheel_env_run(search_dirs, app_data, env):
    env = env.copy()
    env.update(
        {"PIP_USE_WHEEL": "1", "PIP_USER": "0", "PIP_NO_INPUT": "1"})
    wheel = get_wheel(
        distribution="pip",
        version=None,
        for_py_version=f"{
