...
def periodic_update(  # noqa: PLR0913
    ...
    u_log = UpdateLog.from_app_data(
        app_data, distribution, for_py_version)
    if of_version is None:
        for _, group in groupby(
                u_log.versions, key=lambda v: v.wheel.version_tuple[0:2]):
            # use only latest patch version per minor, earlier assumed
            # to be buggy
            ...
            for version in all_patches:
                if wheel is not None and Path(
                        version.filename).name == wheel.name:
                    return wheel
...
def add_wheel_to_update_log(wheel, for_py_version, app_data):
    ...
    logging.debug(
        "adding %s information to %s",
        wheel.name,
        embed_update_log.file)
    u_log = UpdateLog.from_dict(embed_update_log.read())
...
def load_datetime(value):
    return None if value is None else datetime.strptime(
        value, DATETIME_FMT).replace(tzinfo=timezone.utc)
...
class NewVersion:  # noqa: PLW1641
    ...
    def use(self, now, ignore_grace_period_minor=False, ignore_grace_period_ci=False):  # noqa: FBT002
        ...
        if self.source == "periodic" and (
                self.found_date < now -
                GRACE_PERIOD_CI or ignore_grace_period_ci):
    ...
    def __repr__(self) -> str:
        return (
            f"{
                ...
                self.filename}), found_date={
                self.found_date}, " f"release_date={
                    self.release_date}, source={
                        self.source})")
...
class UpdateLog:
    ...
    def from_dict(cls, dictionary):
        ...
        return cls(
            load_datetime(
                dictionary.get("started")), load_datetime(
                dictionary.get("completed")), [
                NewVersion.from_dict(v) for v in dictionary.get(
                    "versions", [])], dictionary.get("periodic"), )

    @classmethod
    def from_app_data(cls, app_data, distribution, for_py_version):
        raw_json = app_data.embed_update_log(
            distribution, for_py_version).read()
...
def trigger_update(distribution, for_py_version, wheel, search_dirs, app_data, env, periodic):  # noqa: PLR0913
    ...
    if debug:
        ...
        # set the returncode here -> no ResourceWarning on main process
        # exit if the subprocess still runs
...
def _run_do_update(  # noqa: C901, PLR0913
    ...
    wheel_filename = None if embed_filename is None else Path(
        embed_filename)
    embed_version = None if wheel_filename is None else Wheel(
        wheel_filename).version_tuple
    app_data = AppDataDiskFolder(app_data) if isinstance(
        app_data, str) else app_data
    search_dirs = [
        Path(p) if isinstance(
            p, str) else p for p in search_dirs]
    ...
    while last is None or not last.use(
            now, ignore_grace_period_ci=True):
    ...
    if content is not None:
        try:
            ...
            return datetime.strptime(
                upload_time, "%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone.utc)
...
def _request_context():
    yield None
    # fallback to non verified HTTPS (the information we request is not
    # sensitive, so fallback)
...
def _pypi_get_distribution_info_cached(distribution):
    if distribution not in _PYPI_CACHE:
        _PYPI_CACHE[distribution] = _pypi_get_distribution_info(
            distribution)
