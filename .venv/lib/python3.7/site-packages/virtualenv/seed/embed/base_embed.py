...
class BaseEmbed(Seeder, ABC):
    ...
    def distribution_to_versions(self) -> dict[str, str]:
        return {
            distribution: getattr(
                self,
                f"{distribution}_version") for distribution in self.distributions() if getattr(
                self,
                f"no_{distribution}") is False and getattr(
                self,
    ...
    def __repr__(self) -> str:
        ...
        if self.extra_search_dir:
            result += f"extra_search_dir={
                ', '.join(
                    str(i) for i in self.extra_search_dir)},"
