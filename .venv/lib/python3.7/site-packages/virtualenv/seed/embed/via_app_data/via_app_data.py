...
class FromAppData(BaseEmbed):
    ...
    def run(self, creator):
        ...
        with self._get_seed_wheels(creator) as name_to_whl:
            ...
            threads = [Thread(target=_install, args=(n, w))
    ...
    def _get_seed_wheels(self, creator):  # noqa: C901
        ...
        def _get(distribution, version):
            ...
            # fallback to download in case the exact version is not
            # available
        ...
        for thread in threads:
            thread.join()
        if fail:
            msg = f"seed failed due to failing to download wheels {
                ', '.join(
                    fail.keys())}"
    ...
    def installer_class(self, pip_version_tuple):
        if self.symlinks and pip_version_tuple and pip_version_tuple >= (
                19, 3):  # symlink support requires pip 19.3+
            return SymlinkPipInstall
        return CopyPipInstall

    def __repr__(self) -> str:
        msg = f", via={
            'symlink' if self.symlinks else 'copy'}, app_data_dir={
            self.app_data}"
        base = super().__repr__()
