...
class PipInstall(ABC):
    ...
    def install(self, version_info):
        ...
        logging.debug(
            "generated console scripts %s", " ".join(
    ...
    def _generate_new_files(self):
        ...
        # inject a no-op root element, as workaround for bug in
        # https://github.com/pypa/pip/issues/7226
    ...
    def _uninstall_previous_version(self):
        ...
        paths = (p for p in in_folders if p.stem.split(
            "-")[0] == dist_name and p.suffix == ".dist-info" and p.is_dir())
    ...
    def _uninstall_dist(dist):
        ...
        logging.debug(
            "uninstall existing distribution %s from %s",
            dist.stem,
            dist_base)
        ...
        for name in ((i.split(",")[0] for i in record.read_text(
                encoding="utf-8").splitlines()) if record.exists() else ()):
            path = dist_base / name
        ...
        for path in sorted(
                paths):  # actually remove stuff in a stable order
            if path.exists():
...
class ScriptMakerCustom(ScriptMaker):
    ...
    def _write_script(
            ...
            ext):
        names.add(
            f"{self._name}{self.version_info[0]}.{self.version_info[1]}")
