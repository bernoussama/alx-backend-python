...
class SymlinkPipInstall(PipInstall):
    ...
    def _generate_new_files(self):
        ...
        cmd = [str(self._creator.exe), "-m",
               "compileall", str(self._image_dir)]
        process = Popen(cmd, stdout=PIPE, stderr=PIPE)
        process.communicate()
        # the root pyc is shared, so we'll not symlink that - but still
        # add the pyc files to the RECORD for close
        ...
        # remove files that are within the image folder deeper than one
        # level (as these will be not linked directly)
    ...
    def _fix_records(self, new_files):
        ...
        extra_record_data_str = self._records_text(
            sorted(new_files, key=str))
        (self._dist_info /
         "RECORD").write_text(extra_record_data_str, encoding="utf-8")
