...
class Parser:
    ...
    # stream    ::= STREAM-START implicit_document? explicit_document* STREAM-END
    # implicit_document ::= block_node DOCUMENT-END*
    # explicit_document ::= DIRECTIVE* DOCUMENT-START block_node?
    # DOCUMENT-END*
    ...
    def parse_document_start(self):
        ...
        if not self.check_token(StreamEndToken):
            ...
            if not self.check_token(DocumentStartToken):
                raise ParserError(
                    None, None, "expected '<document start>', but found %r" %
                    self.peek_token().id, self.peek_token().start_mark)
            token = self.get_token()
        ...
        while self.check_token(DirectiveToken):
            ...
            if token.name == 'YAML':
                ...
                if handle in self.tag_handles:
                    raise ParserError(
                        None, None, "duplicate tag handle %r" %
                        handle, token.start_mark)
    ...
    def parse_block_sequence_entry(self):
        ...
        if not self.check_token(BlockEndToken):
            token = self.peek_token()
            raise ParserError("while parsing a block collection",
                              self.marks[-1],
        ...
        if not self.check_token(FlowSequenceEndToken):
            if not first:
                if self.check_token(FlowEntryToken):
                    ...
                    token = self.peek_token()
                    raise ParserError("while parsing a flow sequence",
                                      self.marks[-1],
    ...
    #                   flow_mapping_entry?
    #                   FLOW-MAPPING-END
    # flow_mapping_entry    ::= flow_node | KEY flow_node? (VALUE
    # flow_node?)?
        ...
        if not self.check_token(FlowMappingEndToken):
            if not first:
                if self.check_token(FlowEntryToken):
                    ...
                    token = self.peek_token()
                    raise ParserError("while parsing a flow mapping",
                                      self.marks[-1],
                                      "expected ',' or '}', but got %r" % token.id,
                                      token.start_mark)
            if self.check_token(KeyToken):
