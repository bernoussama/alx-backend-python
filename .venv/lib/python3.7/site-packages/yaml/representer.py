        ...
        if data_types[0] in self.yaml_representers:
            ...
            for data_type in data_types:
                if data_type in self.yaml_multi_representers:
                    node = self.yaml_multi_representers[data_type](
                        self, data)
                    break
            else:
                if None in self.yaml_multi_representers:
                    node = self.yaml_multi_representers[None](
                        self, data)
    ...
    def represent_sequence(self, tag, sequence, flow_style=None):
        ...
        for item in sequence:
            ...
            if not (
                isinstance(
                    node_item,
                    ScalarNode) and not node_item.style):
                best_style = False
    ...
    def represent_mapping(self, tag, mapping, flow_style=None):
        ...
        for item_key, item_value in mapping:
            ...
            if not (
                isinstance(
                    node_key,
                    ScalarNode) and not node_key.style):
                best_style = False
            if not (
                isinstance(
                    node_value,
                    ScalarNode) and not node_value.style):
                best_style = False
