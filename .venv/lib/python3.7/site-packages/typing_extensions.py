...
if sys.version_info >= (3, 10):
    ...
    def _should_collect_from_parameters(t):
        return isinstance(
            t, (typing._GenericAlias, _types.GenericAlias))
else:
    def _should_collect_from_parameters(t):
...
if sys.version_info >= (3, 10, 1):
    ...
    class _LiteralGenericAlias(typing._GenericAlias, _root=True):
        def __eq__(self, other):
            ...
            these_args_deduped = set(
                _value_and_type_iter(
                    self.__args__))
            other_args_deduped = set(
                _value_and_type_iter(
                    other.__args__))
    ...
    for base in cls.__mro__[:-1]:  # without object
        ...
        for attr in (*base.__dict__, *annotations):
            if (not attr.startswith('_abc_')
                    and attr not in _EXCLUDED_ATTRS):
                attrs.add(attr)
    return attrs
...
if sys.version_info >= (3, 12):
    ...
    class _ProtocolMeta(_ProtocolMetaBase):
                ...
                for base in bases:
                    if not (
                        base in {
                            ...
                            typing.Generic,
                            Protocol,
                            _typing_Protocol} or base.__name__ in _PROTO_ALLOWLIST.get(
                            base.__module__,
                            []) or is_protocol(base)):
                        raise TypeError(
                            f"Protocols can only inherit from other protocols, " f"got {
                                base!r}")
            return abc.ABCMeta.__new__(
        ...
        def __eq__(cls, other):
            ...
            if abc.ABCMeta.__eq__(cls, other) is True:
                return True
            return (
                cls is Protocol and other is getattr(
    ...
    if sys.version_info >= (3, 8):
        ...
        class Protocol(metaclass=_ProtocolMeta):
            ...
            # as the typing module on Python 3.7 doesn't let us subclass
            # typing.Generic!
            ...
            def __class_getitem__(cls, params):
                ...
                if not params and cls is not typing.Tuple:
                    raise TypeError(
                        f"Parameter list to {
                            cls.__qualname__}[...] cannot be empty")
                ...
                if cls is Protocol:
                    ...
                    if not all(isinstance(p, typing.TypeVar)
                        ...
                        while isinstance(params[i], typing.TypeVar):
                            i += 1
                        raise TypeError(
                            "Parameters to Protocol[...] must all be type variables." f" Parameter {
                                i +
                                1} is {
                                params[i]}")
...
if sys.version_info >= (3, 8):
    ...
    def runtime_checkable(cls):
        ...
        if not (
            (isinstance(
                cls,
                _ProtocolMeta) or issubclass(
                cls,
                typing.Generic)) and getattr(
                cls,
                "_is_protocol",
                False)):
...
# Our version of runtime-checkable protocols is faster on Python
# 3.7-3.11
if sys.version_info >= (3, 12):
...
def _ensure_subclassable(mro_entries):
    def inner(func):
        if sys.implementation.name == "pypy" and sys.version_info < (
                3,
                9):
            cls_dict = {
...
if sys.version_info >= (3, 13):
    ...
    class _TypedDictMeta(type):
        def __new__(cls, name, bases, ns, total=True):
            ...
            for base in bases:
                if type(
                        base) is not _TypedDictMeta and base is not typing.Generic:
                    raise TypeError(
            ...
            # the name of the class happens to be "Protocol" (or
            # "_Protocol" on 3.7).
    ...
    def TypedDict(
        ...
        if __fields is _marker or __fields is None:
            ...
            example = f"`{__typename} = TypedDict({
                __typename!r}, {{}})`"
            deprecation_msg = (
            ...
            return (
                f"typing_extensions.Annotated[{
                    typing._type_repr(
                        self.__origin__)}, " f"{
                    ', '.join(
                        repr(a) for a in self.__metadata__)}]")
...
# ParamSpecArgs and ParamSpecKwargs, so only Python 3.10's versions will
# do.
...
if sys.version_info >= (3, 12):
    # dataclass_transform exists in 3.11 but lacks the frozen_default
    # parameter
    ...
    def NamedTuple(__typename, __fields=_marker, **kwargs):
        ...
        if __fields is _marker:
            if kwargs:
                ...
                deprecated_thing = "Failing to pass a value for the 'fields' parameter"
                example = f"`{__typename} = NamedTuple({
                    __typename!r}, [])`"
                deprecation_msg = (
                    "{name} is deprecated and will be disallowed in Python {remove}. "
            ...
            try:
                ...
                raise TypeError(
                    f'Expected an instance of type, not {
                        type(__cls).__name__!r}') from None
...
if sys.version_info >= (3, 11):
    ...
    class NewType:
        ...
        def __mro_entries__(self, bases):
            ...
            # if a user attempts to subclass a NewType instance.
            # bpo-46170
        ...
        if sys.version_info >= (3, 10):
            # PEP 604 methods
            # It doesn't make sense to have these methods on Python
            # <3.10
...
if hasattr(typing, "TypeAliasType"):
    ...
    class TypeAliasType:
        ...
        def __init__(self, name: str, value, *, type_params=()):
            ...
            if def_mod != 'typing_extensions':
                self.__module__ = def_mod
            # Setting this attribute closes the TypeAliasType from
