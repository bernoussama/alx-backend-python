...
class _IndividualSpecifier(BaseSpecifier):
    ...
    def _coerce_version(
            self,
            version: UnparsedVersion) -> ParsedVersion:
    ...
    def contains(
        ...
        operator_callable: CallableOperator = self._get_operator(
            self.operator)
...
class LegacySpecifier(_IndividualSpecifier):
    ...
    def _coerce_version(
            self,
            version: UnparsedVersion) -> LegacyVersion:
...
class Specifier(_IndividualSpecifier):
    ...
    def _compare_equal(
        ...
        if spec.endswith(".*"):
            ...
            # segment.
            split_spec = _version_split(
                spec[:-2])  # Remove the trailing .*
    ...
    def _compare_less_than(
        ...
        if not spec.is_prerelease and prospective.is_prerelease:
            if Version(
                    prospective.base_version) == Version(
                    spec.base_version):
                return False
...
class SpecifierSet(BaseSpecifier):
    ...
    def prereleases(self) -> Optional[bool]:
        ...
        # prereleases, if any of them do we'll return True, otherwise
        # False.
    ...
    def filter(
        ...
        if self._specs:
            for spec in self._specs:
                iterable = spec.filter(
                    iterable, prereleases=bool(prereleases))
            return iterable
        ...
        # releases, and which will filter out LegacyVersion in general.
            ...
            for item in iterable:
                # Ensure that we some kind of Version class for this
                # item.
                if not isinstance(item, (LegacyVersion, Version)):
