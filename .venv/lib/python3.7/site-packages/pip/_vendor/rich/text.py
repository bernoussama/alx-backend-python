...
class Text(JupyterMixin):
    ...
    def on(self, meta: Optional[Dict[str, Any]]
        ...
        meta.update({f"@{key}": value for key,
                    value in handlers.items()})
    ...
    def get_style_at_offset(
        ...
        """
        # TODO: This is a little inefficient, it is only used by full
        # justify
        if offset < 0:
    ...
    def __rich_measure__(
        ...
        max_text_width = max(cell_len(line)
                             for line in lines) if lines else 0
        words = text.split()
        min_text_width = (max(cell_len(word)
                              for word in words) if words else max_text_width)
    ...
    def render(
        ...
        style_map = {
            index: get_style(
                span.style) for index,
            span in enumerated_spans}
        ...
        def get_current_style() -> Style:
            ...
            styles = tuple(style_map[_style_id]
                           for _style_id in sorted(stack))
        ...
        if _overflow != "ignore":
            ...
            if length > max_width:
                if _overflow == "ellipsis":
                    self.plain = set_cell_size(
                        self.plain, max_width - 1) + "…"
                else:
    ...
    def pad(self, count: int, character: str = " ") -> None:
        ...
        """
        assert len(
            character) == 1, "Character must be a string of length 1"
        if count:
    ...
    def with_indent_guides(
        ...
        *,
        ...
        _indent_size = self.detect_indentation(
        ) if indent_size is None else indent_size
        ...
        for line in text.split(allow_blank=True):
            ...
            full_indents, remaining_space = divmod(
                len(indent), _indent_size)
            new_indent = f"{
                indent_line *
                full_indents}{
                ' ' *
            ...
            if blank_lines:
                new_lines.extend(
                    [Text(new_indent, style=style)] * blank_lines)
                blank_lines = 0
            add_line(line)
        if blank_lines:
