...
def _get_attr_fields(
        obj: Any) -> Sequence["_attr_module.Attribute[Any]"]:
...
def _is_dataclass_repr(obj: object) -> bool:
    ...
    # Catching all exceptions in case something is missing on a non
...
def _has_default_namedtuple_repr(obj: object) -> bool:
    ...
    try:
        ...
        # TypeError trapped defensively, in case of object without
        # filename slips through.
        pass
...
def _get_braces_for_array(
        _object: "array[Any]") -> Tuple[str, str, str]:
    return (
        f"array({
            _object.typecode!r}, [", "])", f"array({
            _object.typecode!r})")
...
class _Line:
    ...
    def check_length(self, max_length: int) -> bool:
        ...
        start_length = (len(self.whitespace) +
                        cell_len(self.text) +
                        cell_len(self.suffix))
...
def _is_namedtuple(obj: Any) -> bool:
    ...
    try:
        ...
        # Being very defensive - if we cannot get the attr then its not
        # a namedtuple
        return False
...
def traverse(
    ...
    def _traverse(obj: Any, root: bool = False, depth: int = 0) -> Node:
        ...
        if rich_repr_result is not None:
            ...
            class_name = obj.__class__.__name__
            if reached_max_depth:
                # If we've reached the max depth, we still show the
                # class name, but not its contents
                node = Node(
                ...
                for last, (key, value) in loop_last(
                        obj._asdict().items()):
