...
def valid_string_length(
        label: Union[bytes, str], trailing_dot: bool) -> bool:
...
def check_bidi(label: str, check_ltr: bool = False) -> bool:
    # Bidi rules should only be applied if string contains RTL
    # characters
    bidi_label = False
...
def valid_contexto(
    ...
    if cp_value == 0x00b7:
        if 0 < pos < len(label) - 1:
            if ord(label[pos - 1]
                   ) == 0x006c and ord(label[pos + 1]) == 0x006c:
                return True
        return False
