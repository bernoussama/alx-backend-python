...
class PythonLexer(RegexLexer):
    ...
    def innerstring_rules(ttype):
        return [
            # the old style '%s' % (...) string formatting (still valid
            # in Py3)
            (r'%(\(\w+\))?[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?'
    ...
    tokens = {
        ...
        'soft-keywords-inner': [
            # optional `_` keyword
            (r'(\s+)([^\n_]*)(_\b)',
             bygroups(Text, using(this), Keyword)),
            default('#pop')
        ],
        'builtins': [
            (words((
            ...
            if line.startswith('>>> ') or line.startswith('... '):
                ...
                if (line.startswith('Traceback (most recent call last):')
                    ...
                    if not (
                        ...
                        tb = 0
                        for i, t, v in tblexer.get_tokens_unprocessed(
                                curtb):
                            yield tbindex + i, t, v
                        curtb = ''
                else:
...
class Python2TracebackLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            ...
            # The optional ^C allows us to catch keyboard interrupt
            # signals.
            (r'^(\^C)?(Traceback.*\n)',
    ...
    tokens = {
        ...
        'strings': [
            (r'%(\([a-zA-Z0-9]+\))?[-#0 +]*([0-9]+|[*])?(\.([0-9]+|[*]))?'
             '[hlL]?[E-GXc-giorsux%]', String.Interpol),
            (r'[^\\\'"%\n]+', String),
            # quotes, percents and backslashes must be parsed one at a
            # time
            (r'[\'"\\]', String),
            # unhandled string formatting sign
            (r'%', String)
...
class DgLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            ...
            (r"(?i)(br|r?b?)'", String, combined(
                'stringescape', 'sqs', 'string')),
            (r'(?i)(br|r?b?)"', String, combined(
                'stringescape', 'dqs', 'string')),
...
class NumPyLexer(PythonLexer):
    ...
    def analyse_text(text):
        ...
        return (
            shebang_matches(
                text,
                r'pythonw?(3(\.\d)?)?') or 'import ' in ltext) and (
