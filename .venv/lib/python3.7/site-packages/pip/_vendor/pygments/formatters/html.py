...
class HtmlFormatter(Formatter):
    ...
    **Subclassing the HTML formatter**
    ...
    def __init__(self, **options):
        ...
        self.cssstyles = self._decodeifneeded(
            options.get('cssstyles', ''))
        self.prestyles = self._decodeifneeded(
            options.get('prestyles', ''))
        self.cssfile = self._decodeifneeded(options.get('cssfile', ''))
        self.noclobber_cssfile = get_bool_opt(
            options, 'noclobber_cssfile', False)
        self.tagsfile = self._decodeifneeded(
            options.get('tagsfile', ''))
        ...
        self.anchorlinenos = get_bool_opt(
            options, 'anchorlinenos', False)
        self.hl_lines = set()
    ...
    def _create_stylesheet(self):
        ...
        for ttype, ndef in self.style:
            ...
            if ndef['bgcolor']:
                style += 'background-color: %s; ' % webify(
                    ndef['bgcolor'])
            if ndef['border']:
                style += 'border: 1px solid %s; ' % webify(
                    ndef['border'])
            if style:
                t2c[ttype] = name
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # would misalign the line numbers. Hence we emit a separate row
        # for it.
        filename_tr = ""
        if self.filename:
            filename_tr = (
    ...
    def _wrap_inlinelinenos(self, inner):
        # need a list of lines since we need the width of a single
        # number :(
