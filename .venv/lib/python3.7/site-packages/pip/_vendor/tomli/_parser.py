    ...
    *,
    ...
    if not error_on.isdisjoint(src[pos:new_pos]):
        while src[pos] not in error_on:
            pos += 1
        raise suffixed_err(
            src, pos, f"Found invalid character {
                src[pos]!r}")
    return new_pos
...
def create_dict_rule(src: str, pos: Pos,
                     out: Output) -> tuple[Pos, Key]:
    pos += 1  # Skip "["
    pos = skip_chars(src, pos, TOML_WS)
    ...
    try:
        ...
        raise suffixed_err(
            src, pos, "Cannot overwrite a value") from None
...
def create_list_rule(src: str, pos: Pos,
                     out: Output) -> tuple[Pos, Key]:
    pos += 2  # Skip "[["
    pos = skip_chars(src, pos, TOML_WS)
    pos, key = parse_key(src, pos)
    ...
    # Free the namespace now that it points to another empty list
    # item...
...
def key_value_rule(
    ...
    for cont_key in relative_path_cont_keys:
        # Check that dotted key syntax does not redefine an existing
        # table
...
def parse_inline_table(src: str, pos: Pos,
    ...
    while True:
        ...
        if key_stem in nest:
            raise suffixed_err(
                src, pos, f"Duplicate inline table key {
                    key_stem!r}")
        nest[key_stem] = value
        pos = skip_chars(src, pos, TOML_WS)
        c = src[pos: pos + 1]
...
def parse_basic_str_escape(
    ...
    try:
        ...
        raise suffixed_err(
            src, pos, "Unescaped '\\' in a string") from None
...
def parse_basic_str(src: str, pos: Pos, *                    , multiline: bool) -> tuple[Pos, str]:
    ...
    while True:
        try:
            ...
            raise suffixed_err(
                src, pos, "Unterminated string") from None
        if char == '"':
            if not multiline:
...
def parse_value(  # noqa: C901
    ...
    # IMPORTANT: order conditions based on speed of checking and
    # likelihood
    ...
    if datetime_match:
        try:
            ...
            raise suffixed_err(
                src, pos, "Invalid date or datetime") from e
...
def make_safe_parse_float(parse_float: ParseFloat) -> ParseFloat:
    ...
    """
    # The default `float` callable never returns illegal types. Optimize
    # it.
