...
def counted_array(
    ...
    *,
    ...
    intExpr.add_parse_action(
        count_field_parse_action,
        call_during_try=True)
...
def one_of(
    ...
    *,
    ...
    if useRegex:
        ...
        try:
            if all(len(sym) == 1 for sym in symbols):
                ...
                patt = "[{}]".format(
                    "".join(
                        _escape_regex_range_chars(sym) for sym in symbols))
            else:
            ...
            if caseless:
                ...
                ret.add_parse_action(
                    lambda s, l, t: symbol_map[t[0].lower()])
    ...
    return MatchFirst(parseElementClass(sym)
                      for sym in symbols).set_name(" | ".join(symbols))
...
def original_text_for(
        ...
        as_string: bool = True,
        *,
        asString: bool = True) -> ParserElement:
...
def _makeTags(
    ...
    # add start<tagname> results name in parse action now that ungrouped
    # names are not reported at two levels
...
_htmlEntityMap = {
    k.rstrip(";"): v for k,
    v in html.entities.html5.items()}
...
def infix_notation(
    ...
    prints::
        ...
        [[['-', 2], '-', ['-', 11]]]
    """
    # captive version of FollowedBy that does not do parse actions or
    # capture results names
    ...
    for i, operDef in enumerate(op_list):
        ...
        if arity == 3:
            if not isinstance(opExpr, (tuple, list)
                              ) or len(opExpr) != 2:
                raise ValueError(
