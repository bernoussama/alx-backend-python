    ...
    if shutil.which("rustc") is not None:
        ...
        try:
            ...
            if rustc_output.startswith(b"rustc "):
                ...
                data["rustc_version"] = rustc_output.split(b" ")[
                    1].decode()
...
class LocalFSAdapter(BaseAdapter):
    def send(
        ...
        try:
            ...
            resp.raw = io.BytesIO(
                f"{resp.reason}: {exc}".encode("utf8"))
        else:
            modified = email.utils.formatdate(
                stats.st_mtime, usegmt=True)
            content_type = mimetypes.guess_type(
                pathname)[0] or "text/plain"
...
class PipSession(requests.Session):
    ...
    def iter_secure_origins(
            self) -> Generator[SecureOrigin, None, None]:
