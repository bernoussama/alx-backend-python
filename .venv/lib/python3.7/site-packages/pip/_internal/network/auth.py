...
class MultiDomainBasicAuth(AuthBase):
    def __init__(self, prompting: bool = True,
    ...
    def _get_new_credentials(
        ...
        if index_url and index_url_user_password[0] is not None:
            ...
            if username is not None and password is not None:
                logger.debug(
                    "Found credentials in index url for %s", netloc)
        ...
        if allow_netrc:
            ...
            if netrc_auth:
                logger.debug(
                    "Found credentials in netrc for %s", netloc)
                return netrc_auth
        ...
        if allow_keyring:
            ...
            if kr_auth:
                logger.debug(
                    "Found credentials in keyring for %s", netloc)
                return kr_auth
    ...
    def _get_url_and_credentials(
        ...
        # the username in the index url, but the password comes from
        # keyring.
    ...
    def _should_save_password_to_keyring(self) -> bool:
        if get_keyring_provider() is None:
            return False
        return ask(
            "Save credentials to keyring [y/N]: ", ["y", "n"]) == "y"
    ...
    def handle_401(self, resp: Response, **kwargs: Any) -> Response:
        ...
        # We are not able to prompt the user so simply return the
        # response
