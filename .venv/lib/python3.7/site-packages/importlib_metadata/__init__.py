...
class Sectioned:
    ...
    def read(text, filter_=None):
        ...
        name = None
        for value in lines:
            section_match = value.startswith(
                '[') and value.endswith(']')
            if section_match:
                name = value.strip('[]')
                continue
    ...
    def _from_text(text):
        return (
            EntryPoint(
                ...
                group=item.name) for item in Sectioned.section_pairs(
                text or ''))
...
class DeprecatedNonAbstract:
    def __new__(cls, *args, **kwargs):
        all_names = {name for subclass in inspect.getmro(
            cls) for name in vars(subclass)}
        abstract = {
            name
...
class Distribution(DeprecatedNonAbstract):
    ...
    def files(self) -> Optional[List[PackagePath]]:
        ...
        def skip_missing_files(package_paths):
            return list(
                filter(
                    lambda path: path.locate().exists(),
                    package_paths))
    ...
    def __init__(self, path: FastPath):
        ...
        for child in path.children():
            ...
            if low.endswith((".dist-info", ".egg-info")):
                ...
                self.eggs[legacy_normalized].append(
                    path.joinpath(child))
...
def packages_distributions() -> Mapping[str, List[str]]:
    ...
    for dist in distributions():
        for pkg in _top_level_declared(
                dist) or _top_level_inferred(dist):
...
def _top_level_inferred(dist):
    opt_names = set(
        map(_get_toplevel_name, always_iterable(dist.files)))
