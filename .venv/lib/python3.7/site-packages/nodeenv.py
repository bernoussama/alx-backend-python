...
def is_x86_64_musl():
    return sysconfig.get_config_var(
...
def download_node_src(node_url, src_dir, args):
    ...
    if is_WIN or is_CYGWIN:
        ...
        def member_name(s): return s  # noqa: E731
    else:
...
def urlopen(url):
    ...
    headers = {
        'User-Agent': 'nodeenv/%s (%s)' % (nodeenv_version, home_url)}
...
def build_node_from_src(env_dir, src_dir, node_src_dir, args):
    ...
    if getattr(sys.version_info, 'major', sys.version_info[0]) > 2:
        ...
        logger.debug(
            ' * Temporarily pointing python to %s',
            python2_path)
    ...
    callit(
        ...
        env)
    logger.info('.', extra=dict(continued=True))
    callit([make_cmd + ' install'],
           args.verbose, True, node_src_dir, env)
...
def install_npm_win(env_dir, src_dir, args):
    ...
    if is_CYGWIN:
        ...
        writefile(join(env_dir, 'bin', 'npm'),
                  urlopen(npm_bin_url).read())
...
def install_packages(env_dir, args):
    ...
    real_npm_ver = args.npm if args.npm.count(
        ".") == 2 else args.npm + ".0"
...
def install_activate(env_dir, args):
    ...
    prompt = args.prompt or '(%s)' % os.path.basename(
        os.path.abspath(env_dir))

    if args.node == "system":
        env = os.environ.copy()
        env.update(
            {'PATH': remove_env_bin_from_path(env['PATH'], bin_dir)})
    ...
    for name, content in files.items():
        ...
        content = content.replace(
            '__BIN_NAME__', os.path.basename(bin_dir))
        ...
        if is_CYGWIN:
            ...
            content = content.replace(
                '__NPM_CONFIG_PREFIX__', cyg_bin_dir[0])
        else:
...
def get_node_versions():
    return [dct['version'].lstrip('v')
            for dct in _get_versions_json()][::-1]
