...
def path_to_cache_dir(path, use_abspath=True):
    ...
    d, p = os.path.splitdrive(
        os.path.abspath(path) if use_abspath else path)
    if d:
        d = d.replace(':', '---')
    p = p.replace(os.sep, '--')
    return d + p + '.cache'
...
class Sequencer(object):
    ...
    def strong_connections(self):
        ...
        def strongconnect(node):
            # set the depth index for this node to the smallest unused
            # index
            ...
            # If `node` is a root node, pop the stack and generate an
            # SCC
...
def unarchive(archive_filename, dest_dir, format=None, check=True):
    ...
    try:
        ...
        def extraction_filter(member, path):
            ...
            # This is only called if the current Python has tarfile
            # filters
            try:
    ...
    def __init__(self, uri, **kwargs):
        ...
        if timeout is not None:
            ...
            if scheme == 'https':
                ...
                tcls = Transport
            kwargs['transport'] = t = tcls(
                timeout, use_datetime=use_datetime)
            self.transport = t
...
class Configurator(BaseConfigurator):
    ...
    def configure_custom(self, config):
        ...
        items = [(k, convert(config[k]))
                 for k in config if valid_ident(k)]
        kwargs = dict(items)
            ...
            if 'distutils' in sections:
                ...
                if _servers == []:
                    ...
                    for server in _servers:
                        ...
                        result['username'] = config.get(
                            server, 'username')

                        # optional params
                        for key, default in (('repository', self.DEFAULT_REPOSITORY), (
                                'realm', self.DEFAULT_REALM), ('password', None)):
