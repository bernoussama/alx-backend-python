...
class Locator(object):
    ...
    # instance to a list of tuples (pyver, abi, arch) which you want to
    # match.
    wheel_tags = None
    ...
    def score_url(self, url):
        ...
        is_downloadable = basename.endswith(
            ...
            for ext in self.downloadable_extensions:
                if path.endswith(ext):
                    ...
                    if not t:  # pragma: no cover
                        logger.debug(
                            'No match for project/version: %s', path)
                    else:
                        name, version, pyver = t
                        if not project_name or same_project(
                                project_name, name):
                            result = {
                                'name': name, 'version': version, 'filename': filename, 'url': urlunparse(
    ...
    def locate(self, requirement, prereleases=False):
        ...
        if len(versions) > 2:  # urls and digests keys are present
            ...
            for k in versions:
                ...
                try:
                    ...
                    logger.warning(
                        'error matching %s with %r', matcher, k)
                    pass  # slist.append(k)
            if len(slist) > 1:
        ...
        if result:
            ...
            result.download_urls = versions.get(
                'urls', {}).get(version, set())
            d = {}
...
class SimpleScrapingLocator(Locator):
    ...
    def _process_download(self, url):
        ...
        if self.platform_check and self._is_platform_dependent(url):
            ...
            info = self.convert_url_to_download_info(
                url, self.project_name)
                        ...
                        if link not in self._seen:
                            try:
                                ...
                                if (not self._process_download(
                                        link) and self._should_queue(link, url, rel)):
                                    logger.debug(
    ...
    def _get_project(self, name):
        ...
        if data:
            for info in data.get('files', []):
                ...
                result['urls'].setdefault(
                    dist.version, set()).add(
                    info['url'])
        return result
        ...
        while todo:
            ...
            for r in all_reqts:
                ...
                if not providers:
                    ...
                    provider = self.locator.locate(
                        r, prereleases=prereleases)
