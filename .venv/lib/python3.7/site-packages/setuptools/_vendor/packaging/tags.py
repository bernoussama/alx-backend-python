...
class Tag(object):
    ...
    def __str__(self):
        return "{}-{}-{}".format(self._interpreter,
                                 self._abi, self._platform)

    def __repr__(self):
        return "<{self} @ {self_id}>".format(
            self=self, self_id=id(self))
...
def _pypy_tags(py_version, interpreter, abi, platforms):
    for tag in (Tag(interpreter, abi, platform)
                for platform in platforms):
        yield tag
    for tag in (Tag(interpreter, "none", platform)
                for platform in platforms):
        yield tag
...
def _generic_tags(interpreter, py_version, abi, platforms):
    for tag in (Tag(interpreter, abi, platform)
        ...
        yield tag
    if abi != "none":
        tags = (Tag(interpreter, "none", platform_)
                for platform_ in platforms)
        for tag in tags:
            yield tag
...
def _linux_platforms(is_32bit=_32_BIT_INTERPRETER):
    ...
    for name, glibc_version in manylinux_support_iter:
        ...
        platforms = []
    # Support for a later manylinux implies support for an earlier
    # version.
    platforms += [linux.replace("linux", name)
