...
def _check_extra(extra, reqs):
    ...
    if marker and pkg_resources.invalid_marker(marker):
        raise DistutilsSetupError(
            "Invalid environment marker: " + marker)
...
class Distribution(_Distribution):
    ...
    def patch_missing_pkg_info(self, attrs):
        ...
        if dist is not None and not dist.has_metadata('PKG-INFO'):
            dist._version = pkg_resources.safe_version(
                str(attrs['version']))
    ...
    def _parse_config_files(self, filenames=None):
        ...
        if 'global' in self.command_options:
            for (opt, (src, val)
                 ) in self.command_options['global'].items():
    ...
    def _exclude_misc(self, name, value):
        ...
        if old is not None and not isinstance(old, sequence):
            ...
            setattr(
                self, name, [
                    item for item in old if item not in value])
    ...
    def _parse_command_opts(self, parser, args):
        # Remove --with-X/--without-X options when processing command
        # args
    ...
    def handle_display_options(self, option_order):
        ...
        if six.PY2 or self.help_commands:
            return _Distribution.handle_display_options(
                self, option_order)
