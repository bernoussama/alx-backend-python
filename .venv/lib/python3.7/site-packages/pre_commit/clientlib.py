...
def _entry(modname: str) -> str:
    ...
    return f'{
        shlex.quote(
            sys.executable)} -m pre_commit.meta_hooks.{modname}'
...
class NotAllowed(cfgv.OptionalNoDefault):
    def check(self, dct: dict[str, Any]) -> None:
        if self.key in dct:
            raise cfgv.ValidationError(
                f'{self.key!r} cannot be overridden')
...
META_HOOK_DICT = cfgv.Map(
    ...
    *(
        ...
        cfgv.ConditionalOptional(
            key, cfgv.check_any, value, 'id', hook_id)
...
def ordered_load_normalize_legacy_config(
