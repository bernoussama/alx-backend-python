...
def norm_version(version: str) -> str | None:
    if version == C.DEFAULT:  # use virtualenv's default
        return None
    # virtualenv defaults to our exe
...
def in_env(
    ...
    directory = helpers.environment_dir(
        ENVIRONMENT_DIR, language_version)
...
def health_check(prefix: Prefix, language_version: str) -> str | None:
    ...
    # always use uncached lookup here in case we replaced an unhealthy
    # env
    ...
    # made with an older version of virtualenv? skip `base-executable`
    # check
