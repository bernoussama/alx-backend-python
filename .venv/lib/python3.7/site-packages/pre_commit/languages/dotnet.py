...
def install_environment(
    ...
    with clean_path_on_failure(envdir):
        ...
        nupkgs = [x for x in os.listdir(
            nupkg_dir) if x.endswith('.nupkg')]

        if not nupkgs:
            raise AssertionError(
                'could not find any build outputs to install')

        for nupkg in nupkgs:
            with zipfile.ZipFile(os.path.join(nupkg_dir, nupkg)) as f:
                nuspec, = (x for x in f.namelist()
            ...
            if not namespace:
                raise AssertionError(
                    'could not parse namespace from nuspec')
