...
class Store:
    ...
    def _new_repo(
        ...
        with self.exclusive_lock():
            ...
            directory = tempfile.mkdtemp(
                prefix='repo', dir=self.directory)
    ...
    def _complete_clone(
            self, ref: str, git_cmd: Callable[..., None]) -> None:
    ...
    def make_local(self, deps: Sequence[str]) -> str:
        def make_local_strategy(directory: str) -> None:
            for resource in self.LOCAL_RESOURCES:
                resource_dirname, resource_basename = os.path.split(
                    resource)
                contents = resource_text(
                    f'empty_template_{resource_basename}')
                target_dir = os.path.join(directory, resource_dirname)
                target_file = os.path.join(
                    target_dir, resource_basename)
            ...
            # initialize the git repository so it looks more like cloned
            # repos
            def _git_cmd(*args: str) -> None:
                cmd_output_b('git', *args, cwd=directory, env=env)
    ...
    def mark_config_used(self, path: str) -> None:
        ...
        with self.connect() as db:
            ...
            self._create_config_table(db)
            db.execute(
                'INSERT OR IGNORE INTO configs VALUES (?)', (path,))
    ...
    def select_all_repos(self) -> list[tuple[str, str, str]]:
        with self.connect() as db:
            return db.execute(
                'SELECT repo, ref, path from repos').fetchall()
