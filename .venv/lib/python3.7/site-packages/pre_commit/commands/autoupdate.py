...
def _original_lines(
    ...
    idxs = [i for i, line in enumerate(
        lines) if REV_LINE_RE.match(line)]
...
def _write_new_config(path: str,
                      rev_infos: list[RevInfo | None]) -> None:
    ...
    for idx, rev_info in zip(idxs, rev_infos):
        ...
        new_rev_s = yaml_dump(
            {'rev': rev_info.rev}, default_style=match[3])
