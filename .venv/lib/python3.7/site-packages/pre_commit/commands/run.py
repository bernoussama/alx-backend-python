...
def _full_msg(
    ...
    dots = '.' * (cols - _len_cjk(start) -
                  len(postfix) - len(end_msg) - 1)
...
class Classifier:
    ...
    def filenames_for_hook(self, hook: Hook) -> tuple[str, ...]:
        ...
        names = filter_by_include_exclude(
            names, hook.files, hook.exclude)
        names = self.by_types(
            names,
            hook.types,
            hook.types_or,
    ...
    def from_config(
        ...
        if os.altsep == '/' and os.sep == '\\':
            filenames = [f.replace(os.sep, os.altsep)
                         for f in filenames]
...
def _run_single_hook(
    ...
    if hook.id in skips or hook.alias in skips:
        ...
        # print hook and dots first in case the hook takes a while to
        # run
...
def _has_unstaged_config(config_file: str) -> bool:
    ...
    # be explicit, other git errors don't mean it has an unstaged
    # config.
    return retcode == 1
