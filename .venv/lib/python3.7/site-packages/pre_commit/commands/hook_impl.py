...
def _validate_config(
    ...
    if not os.path.isfile(config):
        if skip_on_missing_config or os.getenv(
                'PRE_COMMIT_ALLOW_NO_CONFIG'):
            print(
                f'`{config}` config file not found. Skipping `pre-commit`.')
    ...
    for line in stdin.decode().splitlines():
        ...
        if local_sha == Z40:
            ...
            if not ancestors:
                ...
                roots = set(subprocess.check_output(
                    cmd).decode().splitlines())
